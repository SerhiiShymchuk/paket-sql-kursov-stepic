...
SELECT 
        com.name as company, 
        coalesce (SEC_TO_TIME(sum(cal.duration_sec)), '00:00:00') as duration
    from calls as cal
    left join clients as cl on cl.id = cal.client_id
    right join companies as com on com.id = cl.company_id
    group by com.id
    order by duration;