...
select
    users.last_name,
    users.first_name, 
    date_format(calendar.visit_date, '%d.%m') as visit_time
from users, calendar
where 
    calendar.doctor_id = 9 
    and calendar.client_id = users.id
    and date(calendar.visit_date) = '2017-04-17'
