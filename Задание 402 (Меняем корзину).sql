...
-- get all orders for user 'Александр Дмитриев'
select products.name, orders.status
    ...
    where users.first_name = 'Александр' and users.last_name = 'Дмитриев';

-- add холодильник to shopping cart
...
insert into orders_details (order_id, product_id)
    values 
    (
        (select orders.id from orders order by date desc limit 1),
        (select products.id from products where products.name = 'Холодильник')
    );

--delete from shopping cart 'Пылесос'
delete from orders
    where orders.id = (
        select ord.id from (select * from orders) as ord
        join users on ord.user_id = users.id
        join orders_details on ord.id = orders_details.order_id
        join products on products.id = orders_details.product_id
        where 
            products.name = 'Пылесос' 
            and ord.status = 'new'
            and users.first_name = 'Александр'
