...
SELECT
        DATE_FORMAT(calls.date, '%H:%m') as time,
        CONCAT(m.first_name, ' ', m.last_name) as manager,
        CONCAT(cl.first_name, ' ', cl.last_name) as client,
        com.name as company,
        SEC_TO_TIME(calls.duration_sec) as duration
    from calls
    left join managers as m on m.id = calls.manager_id
    left join clients as cl on cl.id = calls.client_id
    left join companies as com on com.id = cl.company_id
    where date(calls.date) = '2018-04-05'
    order by calls.date, time;