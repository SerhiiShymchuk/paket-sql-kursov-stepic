...
VALUES
    ...
    (
        7,
        'Комплект Mango (джинсы Lonny 2 и косуха)',
        4,
        11900,
        '36,38,M,L,S'
    ),
    ...
    (
        16,
        'Джеггинсы Sasha Rozhdestvenskaya',
        5,
        1691,
        '42,46,48,50'
    ),
...
CREATE FULLTEXT INDEX name ON products(name);
SELECT
    id,
    name,
    price
...
WHERE
    count > 0
    AND (
        find_in_set('36', sizes)
        OR find_in_set('38', sizes)
    )
    AND MATCH(name) AGAINST ('+джинс* +mango' IN boolean MODE);